...
const programs = [
  {
    icon: "🔌",
    title: "IoT Bootcamps",
    desc: "Learn the basics of Internet of Things with hands-on projects, sensors, and real devices. No experience needed!"
  },
  {
    icon: "🖨️",
    title: "3D Printing Workshops",
    desc: "Design, print, and take home your own 3D creations. Explore CAD, slicing, and hands-on printing."
  },
  {
    icon: "🤖",
    title: "Robotics for Beginners",
    desc: "Build and program simple robots. Learn coding, electronics, and teamwork in a fun, supportive environment."
  },
  {
    icon: "🔧",
    title: "Electronics & Soldering Clinics",
    desc: "Get hands-on with circuits, soldering, and troubleshooting. Perfect for tinkerers and curious minds."
  },
  {
    icon: "💡",
    title: "Design Thinking Sessions",
    desc: "Learn creative problem-solving, rapid prototyping, and innovation skills for real-world challenges."
  }
];
  ...
  return (
    <div className="relative overflow-hidden dark:bg-emerald-950 transition-colors">
      ...
      <section className="text-center py-16 md:py-24 bg-gradient-to-br from-emerald-100 via-white to-emerald-50 rounded-2xl shadow-xl mb-12 relative overflow-hidden dark:from-emerald-900 dark:via-emerald-950 dark:to-emerald-900">
        ...
        <h1 className="text-5xl md:text-6xl font-extrabold mb-6 text-emerald-700 drop-shadow-lg tracking-tight animate-fade-in dark:text-emerald-200">Our Programs</h1>
        <p className="text-xl md:text-2xl text-emerald-900 mb-8 max-w-2xl mx-auto animate-fade-in-slow dark:text-emerald-100">
          Dive into hands-on learning, creative problem-solving, and real-world tech skills. There’s something for every maker!
        </p>
      </section>
      {/* Section Divider */}
      ...
      <section className="py-16 md:py-24 bg-white dark:bg-emerald-900 transition-colors">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {programs.map((prog, i) => (
            <div key={prog.title} className={`bg-gradient-to-br from-emerald-50 to-white rounded-3xl shadow-xl p-10 flex flex-col items-center hover:scale-105 hover:shadow-2xl transition-transform duration-200 animate-fade-in${i % 2 === 0 ? "-slow" : ""} dark:from-emerald-800 dark:to-emerald-900 dark:text-emerald-100`}>
              <span className="text-emerald-700 text-6xl mb-4 animate-bounce dark:text-emerald-200">{prog.icon}</span>
              <h3 className="font-bold text-2xl mb-2 text-emerald-800 dark:text-emerald-200">{prog.title}</h3>
              <p className="text-gray-700 text-center mb-6 dark:text-emerald-100">{prog.desc}</p>
              <Link
                to="/contact"
                className="bg-gradient-to-r from-emerald-700 to-emerald-500 text-white px-6 py-2 rounded-full hover:from-emerald-800 hover:to-emerald-600 font-semibold shadow-lg transition dark:from-emerald-900 dark:to-emerald-700 dark:text-emerald-100"
              >
                Get Started
              </Link>
            </div>
      ...
      <section className="text-center py-12 bg-emerald-50 dark:bg-emerald-950 transition-colors">
        <h2 className="text-3xl font-bold text-emerald-700 mb-4 dark:text-emerald-200">Ready to Make Something Amazing?</h2>
        <p className="text-lg text-gray-700 mb-6 dark:text-emerald-100">Contact us to join a program, ask questions, or suggest a new workshop. We can’t wait to build with you!</p>
        <Link
          to="/contact"
